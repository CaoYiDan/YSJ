...
@implementation DOPDropDownMenu {
    ...
    UITextField *_lowFiled;
    UITextField *_highFiled;
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (NSInteger index = 0; index < _numOfMenu; ++index) {
        [_currentSelectRowArray addObject:@(-1)];
    }
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
         ...
         NSArray *titleArr = @[@"位置",@"培训分类",@"价格",@"综合排序"];
        titleString = titleArr[i];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    self.ifPriceSifting=tapIndex==2;
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        tableViewHeight+=_ifPriceSifting?180:0;
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    ...
    return  self.ifPriceSifting?180:0;
}

- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    UIView *footer = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kWindowW, 180)];
    footer.backgroundColor = KWhiteColor;
    UILabel *title = [[UILabel alloc]initWithFrame:CGRectMake(0, 10, kWindowW, 40)];
    title.text = @"价格";
    title.textAlignment = NSTextAlignmentCenter;
    [footer addSubview:title];
    
    CGFloat priceTextWidth = 90;
    UITextField *lowPrice = [[UITextField alloc]initWithFrame:CGRectMake(kWindowW/2-15-priceTextWidth, 50, priceTextWidth, 30)];
    lowPrice.borderStyle = UITextBorderStyleRoundedRect;
    lowPrice.backgroundColor = RGB(243, 243, 243);
    _lowFiled = lowPrice;
    lowPrice.placeholder = @"最低价";
    [footer addSubview:lowPrice];
    
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(kWindowW/2-10, 65, 20, 1)];
    line.backgroundColor = [UIColor grayColor];
    [footer addSubview:line];
    
    UITextField *highPrice = [[UITextField alloc]initWithFrame:CGRectMake(kWindowW/2+15, 50, priceTextWidth, 30)];
    highPrice.borderStyle = UITextBorderStyleRoundedRect;
    _highFiled = highPrice;
    highPrice.backgroundColor = RGB(243, 243, 243);
    highPrice.placeholder = @"最高价";
    [footer addSubview:highPrice];
    
    UIButton *reset = [FactoryUI createButtonWithtitle:@"重置" titleColor:KWhiteColor imageName:nil backgroundImageName:nil target:self selector:@selector(reset)];
    reset.layer.cornerRadius = 5;
    reset.clipsToBounds = YES;
    reset.backgroundColor = RGB(204, 204, 204);
    reset.frame = CGRectMake(20, 100, 100, 40);
    [footer addSubview:reset];
    
    UIButton *sure = [FactoryUI createButtonWithtitle:@"确认" titleColor:KWhiteColor imageName:nil backgroundImageName:nil target:self selector:@selector(sure)];
    sure.backgroundColor = KMainColor;
    sure.frame = CGRectMake(20+15+100, 100, kWindowW-40-15-100, 40);
...
-(void)sure{
    ...
    _currentSelectRowArray[_currentSelectedMenudIndex] = @(-1);
     CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [NSString stringWithFormat:@"%@-%@",_lowFiled.text,_highFiled.text];
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
    [self.delegate priceDidClickSureWithLow:[_lowFiled.text integerValue] high:[_highFiled.text integerValue]];
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    if (_currentSelectedMenudIndex==0 && item==0) {
        //在此处约定好，如果是Item==1000,则说明选择的是”全部“,此时 只返回1级目录的名字
       title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:1000]];
    }else{
