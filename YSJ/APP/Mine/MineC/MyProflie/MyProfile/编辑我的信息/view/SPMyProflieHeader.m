...
-(void)setUser:(SPUser *)user{
    ...
    [_headerImg sd_setImageWithURL:[NSURL URLWithString:user.avatar] placeholderImage:[UIImage imageNamed:@"placeholder2"]];
    ...
    //年龄和星座
    _ageAndConstellation.myText= [NSString stringWithFormat:@"%@ 岁| %@",user.age,user.zodiac];
...
//    }
    ...
    //点赞
    [_zan setTitle:[NSString stringWithFormat:@"%d",user.praiseNum] forState:0];
    
    //签名
    _signTextFiled.text = isEmptyString(user.signature)?@"这家伙很懒，没有个性签名~":user.signature;
