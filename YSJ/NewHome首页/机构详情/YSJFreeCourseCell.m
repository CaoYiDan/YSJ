...
@implementation YSJFreeCourseCell
{
    UIImageView *_img;
    UILabel *_leftCount;
    UILabel *_name;
...
- (void)initUI{
    ...
    goToLisen = [[UIButton alloc]init];
    goToLisen.layer.cornerRadius = 4;
    goToLisen.clipsToBounds = YES;
    [goToLisen setTitle:@"去试听" forState:0];
    [goToLisen setTitleColor:KWhiteColor forState:UIControlStateNormal];
    goToLisen.userInteractionEnabled = NO;
    [goToLisen setBackgroundImage:[UIImage imageNamed:@"more_btn"] forState:0];
    goToLisen.titleLabel.font = font(12);
    [goToLisen addTarget:self action:@selector(goFree) forControlEvents:UIControlEventTouchDown];
    [self addSubview:goToLisen];
    [goToLisen mas_makeConstraints:^(MASConstraintMaker *make) {
