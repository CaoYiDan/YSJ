...
static NSString *const ID = @"YSJSpellCell";
@implementation YSJSpellCell
{
    ...
    UILabel * _leftTimeTitle;
    UIView *_leftTimeView;
    
}
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    
    YSJSpellCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    
    if (!cell) {
        cell = [[YSJSpellCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
    }
...
-(void)refreshTimeWithModel:(YSJSpellListModel*)timeModel{
    ...
    _leftTimeTitle.text = [NSString stringWithFormat:@"剩余 %@ : %@ : %@",self.hourLabel.text,self.miniteLabel.text,self.secondLabel.text];
}
//刷新时间
- (void)updateTime {
...
- (void)setCell {
    ...
    _leftTimeView.clipsToBounds = YES;
    ...
    goToSpell.userInteractionEnabled = NO;
