...
-(void)next{
    ...
    NSArray *keyArr = @[@"",@"feaatures",@"",@"suitable_range",@"",@"lables"];
    ...
    //课时数
    [dic setObject:[self.courseNum componentsSeparatedByString:@":"][1] forKey:@"times"];
    
    //需求种类(私教、机构)
    NSDictionary *d = @{@(1):@"精品课",@(0):@"名师课",@(2):@"试听课"};
    [dic setObject:d[@(self.selectedBtn.tag)] forKey:@"type"];
    //token
    ...
    if (self.selectedBtn.tag==2) {
        ...
        //原价
        [dic setObject:@(0) forKey:@"old_price"];
