...
- (ZLPhotoActionSheet *)getPas
{
    ...
    actionSheet.allowSelectLivePhoto = YES;
...
-(void)save{
    ...
    if (isEmptyString(self.textView.text) || self.photos.count==0 || isEmptyString(self.classTime.rightSubTitle) || isEmptyString(self.classNums.rightSubTitle)) {
        Toast(@"请填写完整信息");
        return;
    }
    
    !self.block?:self.block(self.textView.text,self.photos,self.classTime.rightSubTitle,self.classNums.rightSubTitle);
    [self.navigationController popViewControllerAnimated:YES];
}
@end
