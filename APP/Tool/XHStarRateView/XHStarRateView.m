...
//#define ForegroundStarImage @"progress2"
...
{
    NSString *ForegroundStarImage;
    NSString *BackgroundStarImage;
}
#pragma mark - 代理方式
...
-(instancetype)initWithFrame:(CGRect)frame numberOfStars:(NSInteger)numberOfStars rateStyle:(RateStyle)rateStyle isAnination:(BOOL)isAnimation foreBackgroundStarImage:(NSString *)foreBackgroundStarImage backgroundStarImage:(NSString *)backgroundStarImage finish:(finishBlock)finish{
    if (self = [super initWithFrame:frame]) {
        ForegroundStarImage = foreBackgroundStarImage;
        BackgroundStarImage = backgroundStarImage;
        _numberOfStars = numberOfStars;
        _rateStyle = rateStyle;
        _isAnimation = isAnimation;
        _complete = ^(CGFloat currentScore){
...
-(void)setStarLeave:(CGFloat)realStarScore{
