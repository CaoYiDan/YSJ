...
@property (nonatomic, strong) SZScrollView *photoScrollView;
...
#pragma mark - 懒加载
- (SZScrollView *)photoScrollView {
    if (!_photoScrollView) {
        CGRect frame = self.view.bounds;
        frame.origin.x   -= kPhotoViewPadding;
        frame.size.width += (2 * kPhotoViewPadding);
        _photoScrollView = [[SZScrollView alloc] initWithFrame:frame];
...
- (void)browserNoneShow {
    ...
    [UIView animateWithDuration:kAnimationDuration animations:^{
        ...
        self.isShow = YES;
        [photoView setupPhoto:photo];
        [self currentPlayViewPlay:self.currentIndex];
...
//    NSLog(@"self.photoScrollView.contentOffset-=====%@",NSStringFromCGPoint(self.photoScrollView.contentOffset));
    self.photoScrollView.contentSize = CGSizeMake(photoScrollW * self.photos.count, 0);
//    NSLog(@"self.photoScrollView.contentSize-=====%@",NSStringFromCGSize(self.photoScrollView.contentSize));
...
- (void)handlePanZoomScale:(UIPanGestureRecognizer *)panGesture {
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [photoView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                obj.transform = CGAffineTransformConcat(translation, scale);
            }];
...
- (void)showSlideDismissAnimationWithPoint:(CGPoint)point {
    ...
    [UIView animateWithDuration:kAnimationDuration animations:^{
        ...
        [photoView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
         obj.transform = CGAffineTransformMakeTranslation(0, toTranslationY);
        }];
...
-(void)currentPlayViewPlay: (NSInteger)index {
    GKPhoto *photo = self.photos[index];
    if (photo.isVideo && _isShow) {
