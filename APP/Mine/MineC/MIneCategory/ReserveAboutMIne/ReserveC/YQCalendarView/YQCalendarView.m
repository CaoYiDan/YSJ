...
-(UIColor *)dayColor{
    return _dayColor?_dayColor:[UIColor colorWithRed:107/255 green:94/255 blue:253/255 alpha:1];
}
...
-(UIColor *)selectedBackColor{
    return _selectedBackColor?_selectedBackColor:[UIColor whiteColor];
}
...
-(UIImage *)selectedIcon{
    return _selectedIcon?_selectedIcon:[UIImage imageNamed:@"rl"];
}
...
-(UIImage *)preBTNIcon{
    //icon_qian.png
    return _preBTNIcon?_preBTNIcon:[UIImage imageNamed:@"icon_qian"];
}
              ...
              andFrame:(CGRect)frame
{
    ...
    while ([Monthformater stringFromDate:inputDate].intValue == month){
        ...
        YQCalendarEachDay *eachdayView = [[YQCalendarEachDay alloc]
                                            initWithFrame:CGRectMake(nowX-8,
                                                                     nowY,
                                                                     Width+16,
                                                                     Height)];
...
//把某一天变为未选中状态
-(void)setOneDayWithUnseleted:(NSString *)dayStr{
