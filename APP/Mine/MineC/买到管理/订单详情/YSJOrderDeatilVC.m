...
@property (nonatomic,strong) YSJOrderModel *detailModel;
@end
...
-(void)getListRequestisScu:(void(^)(BOOL isScu))requestisScu{
    ...
    [[HttpRequest sharedClient]httpRequestPOST:YCourseUserBuyDeatil parameters:@{@"token":[StorageUtil getId],@"id":self.model.orderId} progress:nil sucess:^(NSURLSessionDataTask *task, id responseObject, ResponseObject *obj) {
        //        requestisScu(YES);
        NSLog(@"%@",responseObject);
        self.detailModel = [YSJOrderModel mj_objectWithKeyValues:responseObject];
...
-(void)setMiddleView{
    ...
    NSArray *contentArr = @[[NSString stringWithFormat:@"¥%.2f",self.detailModel.amount],[NSString stringWithFormat:@"%d节课",self.model.times],@"0"];
...
-(void)setOrderModel{
    ...
    _name.text = self.detailModel.title;
...
-(void)setBottomView{
    ...
    if ([self.model.order_status isEqualToString:@"待付款"]) {
        ...
        arr = @[@"买家账号",@"订单编号",@"下单时间"];
        contentArr = @[[NSString stringWithFormat:@"%@",[SPCommon changeTelephone:self.model.phone]],[NSString stringWithFormat:@"%@",self.model.order_id],[NSString stringWithFormat:@"%@",[SPCommon getTimeFromTimestamp:self.model.create_time]]];
        
    }else{
        
        arr = @[@"买家账号",@"订单编号",@"支付方式",@"下单时间"];
        
        contentArr = @[[NSString stringWithFormat:@"%@",[SPCommon changeTelephone:self.model.phone]],[NSString stringWithFormat:@"%@",self.model.order_id],[NSString stringWithFormat:@"%@",isEmptyString(self.model.checktype)?@"支付宝":self.model.checktype],[NSString stringWithFormat:@"%@",[SPCommon getTimeFromTimestamp:self.model.create_time]]];
    }
...
-(NSArray *)getArrForSell{
    ...
    if ([self.model.order_status isEqualToString:@"待授课"]) {
        ...
        arr = @[@{@"title":@"查看打款进度",@"type":@(1)},@{@"title":@"联系买家",@"type":@(0)}];
